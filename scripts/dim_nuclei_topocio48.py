...
save_dir = ('/Users/greg/Dropbox (HMS)/Baker_QC_2021/script_output/dim_nuclei_topacio48')
if not os.path.exists(save_dir):
    os.makedirs(save_dir)

# read raw single-cell data
data = pd.read_csv(
    '/Volumes/My Book/cylinter_input/topacio_mcupdated_cellring/csv/' +
    f'unmicst-{sample}_cellRingMask.csv', index_col=0
    )
...
plt.imshow(dna_crop, alpha=0.6, cmap='magma', vmax=dna.max())
cbar = plt.colorbar(format=ticker.FuncFormatter(fmt))
cbar.set_alpha(1.0)
cbar.set_label('Hoechst per cell', labelpad=20, rotation=270)
cbar.draw_all()  # enusure cbar uses alpha=1.0
plt.grid(False)
